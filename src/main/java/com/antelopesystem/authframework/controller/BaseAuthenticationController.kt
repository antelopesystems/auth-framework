...
abstract class BaseAuthenticationController(
    ...
    fun initializeLogin(@RequestBody(required = false) body: String, @RequestParam tokenType: TokenType, request: HttpServletRequest): ResultRO<*> {
        return wrapResult {
            return@wrapResult authenticationService.initializeLogin(MethodRequestPayload(entityType, request.parameterMap, body), tokenType)
        }
    }
    ...
    fun doLogin(@RequestBody(required = false) body: String, @RequestParam tokenType: TokenType, request: HttpServletRequest): ResultRO<*> {
        return wrapResult {
            return@wrapResult authenticationService.doLogin(MethodRequestPayload(entityType, request.parameterMap, body), tokenType)
        }
    }
    ...
    fun initializeRegistration(@RequestBody(required = false) body: String, @RequestParam tokenType: TokenType, request: HttpServletRequest): ResultRO<*> {
        return wrapResult {
            return@wrapResult authenticationService.initializeRegistration(MethodRequestPayload(entityType, request.parameterMap, body), tokenType)
        }
    }
    ...
    fun doRegister(@RequestBody(required = false) body: String, @RequestParam tokenType: TokenType, request: HttpServletRequest): ResultRO<*> {
        return wrapResult {
            return@wrapResult authenticationService.doRegister(MethodRequestPayload(entityType, request.parameterMap, body), tokenType)
        }
    }
    ...
    fun initializeForgotPassword(@RequestBody(required = false) body: String, request: HttpServletRequest): ResultRO<*> {
        return wrapVoidResult {
            authenticationService.initializeForgotPassword(MethodRequestPayload(entityType, request.parameterMap, body))
        }
    }
    ...
    fun redeemForgotPasswordToken(@PathVariable token: String, @RequestParam newPassword: String): ResultRO<*> {
        return wrapVoidResult {
            authenticationService.redeemForgotPasswordToken(token, newPassword, entityType)
        }
    }
    ...
    fun changePassword(@RequestBody(required = false) body: String, @RequestParam newPassword: String, request: HttpServletRequest): ResultRO<*> {
        return wrapResult {
            return@wrapResult authenticationService.changePassword(MethodRequestPayload(entityType, request.parameterMap, body), newPassword, entityType)
        }
    }
    ...
    fun getAvailableMethodsForEntity(): ResultRO<*> {
        return wrapResult {
            return@wrapResult authenticationService.getAvailableMethods(entityType)
        }
    }
}
